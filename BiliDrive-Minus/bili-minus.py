...
def warn(e: Exception):
    ...
    print(
        "Error:[%s]; function:[%s] in file [%s]: line [%i]"
        % (e, func.co_name, os.path.basename(func.co_filename), func.co_firstlineno)
    )
...
def meta_string(url):
    return (
        ("bdrive://" + re.findall(r"[a-fA-F0-9]{40}", url)[0])
        if re.match(r"^http(s?)://i0.hdslb.com/bfs/album/[a-fA-F0-9]{40}.[\S]*", url)
        else url
    )
...
class ImageConverter(object):
    ...
    def _image_unpack(self, packed_data):
        ...
        if packed_data[-1] == 255:  # type(bytes[-1])==int ==> 255=b'\xff'
            ...
            data,raw_sha1_17=packed_data[:-(1+17)],packed_data[-(1+17):-1]
            new_sha1=calc_sha1(data)
            if new_sha1[:17]==raw_sha1_17:
                return data
            else:
                raise ValueError(f"图片中存储sha1：{raw_sha1_17},而图中数据计算得到sha1: {new_sha1}")
...
class CheckDuplicate(object):
    ...
    def check_duplicate(file_name, json_name=None):
        ...
        if first_4mb_sha1 in history:
            ...
            log(
                f"文件已于{time.strftime('%Y-%m-%d %H:%M:%S',loc_time)}上传, \
...
class Block(object):
    ...
    def save(self, file_name, url, duplicate_key=None):
        ...
        if not duplicate_key:
            first_4mb_sha1 = calc_sha1(
                read_in_chunk(file_name, chunk_size=4 * 1024 * 1024, chunk_number=1), hexdigest=True
            )
        # todo mkdir for json
...
class UploadClass(object):
    ...
    def _meta_upload(self):
        ...
        for i in range(5):
            ...
            if response and response["code"] == 0:
                ...
                log(
                    f"{meta_dict['filename']} ({size_string(meta_dict['size'])}) 上传完毕,\
    ...
    def _commit_upload(self, data):
        ...
        files = {
            ...
            "file_up": (f"{int(time.time() * 1000)}." + self.args.img_format.lower(), data),
        }
        try:
            response = requests.post(
                api, headers=headers, cookies=self.cookies, data=payload, files=files, timeout=300
            ).json()
            return response
        except Exception as e:
            warn(e)
...
class DownloadClass(object):
    ...
    def probe_meta(self):
        ...
        if self.meta_dict:
            ...
            log(
                f"开始下载: {os.path.basename(self.args.file)}({size_string(self.meta_dict['size'])}),\
    ...
    def _download_meta(self, link):
        full_meta = self._get_format(link)
        try:
    ...
    def resist_check(self):
        ...
        if not os.path.exists(file_name):
            ...
            os.path.getsize(file_name) == self.meta_dict["size"]
            ...
            with open(file_name, "rb") as f:
                for index, block_dict in enumerate(self.meta_dict["block"]):
                    ...
                    if not (calc_sha1(f.read(block_dict["size"]), hexdigest=True) == block_dict["sha1"]):
            ...
            for index in self.block_list:
                ...
                if not self.terminate_flag.is_set():
                    self.thread_pool.append(
                        threading.Thread(target=self._content_download, args=(index, self.meta_dict["block"][index]))
                    )
                    self.thread_pool[-1].start()
                else:
                    log("已终止下载, 等待线程回收")
    ...
    def verify_hash(self):
        try:
            log(
                f"{os.path.basename(self.args.file)} ({size_string(self.meta_dict['size'])}) 下载完毕, 用时{time.time() - self.start_time:.1f}秒, 平均速度{size_string(self.meta_dict['size'] / (time.time() - self.start_time))}/s"
            )
        except Exception as e:
            warn(e)
            return False
...
if __name__ == "__main__":
    ...
    argv=sys.argv
    if len(argv)==4:
        uargs=UArgs(argv[1],argv[2],argv[3])
        upload=UploadClass(uargs)
        upload.run()
    elif len(argv)==2:
        link=argv[1]
